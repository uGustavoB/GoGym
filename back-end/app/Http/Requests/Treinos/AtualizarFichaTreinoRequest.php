...
class AtualizarFichaTreinoRequest extends FormRequest
{
    ...
    {
        $ficha = $this->route('ficha_treino');
        return $this->user()?->can('update', $ficha) ?? false;
    }
    ...
    public function rules(): array
    {
        ...
        $baseRules    = $this->regrasFichaBase();
        $semanasRules = $this->regrasSemanasRotinas($fichaId);

        if ($usuario->personal) {
            $personalId = $usuario->personal->id;
            return array_merge($baseRules, $semanasRules, [
                'aluno_id' => [
                    'required',
                    'integer',
    ...
    private function regrasFichaBase(): array
    {
        return [
    ...
    private function regrasSemanasRotinas(?int $fichaId): array
    {
        return [
            ...
            'rotinas.*.id' => [
                'nullable', 'integer',
                Rule::exists('rotinas_sessoes', 'id')->where('ficha_treino_id', $fichaId),
            ],
